...
var parserMachine = machina.Fsm.extend({
  ...
  newline: function( text, stream, cb ) {
    this.line.pos = 0
    this.line.text = text
    this.stream = stream
    this.cb = cb
  ...
  states: {
    ...
    closed: {
      ...
      search: function() {
        var result = parser.next( this.line.text, this.line.pos )
        ...
        result.next < Infinity ?  this.transition('open') : this.eol()
      }
    },
    ...
    open: {
      ...
      search: function() {
        ...
        var result = parser.next( this.line.text, this.line.pos, [this.blockType] )
...
function stateMachineFactory( transformInline, transformDisplay ) {
  var m = new parserMachine()
  if( transformInline ) m.transform.inline = transformInline
  if( transformDisplay ) m.transform.display = transformDisplay
  return m
}
